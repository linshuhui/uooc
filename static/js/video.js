...
var BigPlayButton = (function (_Button) {
  ...
  BigPlayButton.prototype.buildCSSClass = function buildCSSClass() {
    return 'vjs-big-play-button animated fadeIn';
  };
  ...
  PlaybackRateMenuButton.prototype.createMenu = function createMenu() {
    ...
    if (rates) {
      for (var i = rates.length - 1; i >= 0; i--) {
        menu.addChild(new _playbackRateMenuItemJs2['default'](this.player(), { 'rate': rates[i] + '倍' }));
      }
    }
...
MediaError.defaultMessages = {
  1: '播放失败', // You aborted the media playback
  2: '请检查是否网络原因, 导致视频资源加载失败', //  A network error caused the media download to fail part-way.
  3: '视频格式不支持', // The media playback was aborted due to a corruption problem or because the media used features your browser did not support.
  4: '网络原因或视频格式不支持', // The media could not be loaded, either because the server or network failed or because the format is not supported.
  5: '视频资源解析错误' // The media is encrypted and we do not have the keys to decrypt it.
};
